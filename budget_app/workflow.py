...
async function faToggleInvoices(glCode, el) {
  ...
  html += '</div>';
  html += '<table style="font-size:12px; border-collapse:collapse; background:white; border-radius:6px; overflow:hidden; box-shadow:0 1px 2px rgba(0,0,0,0.05);">';
  ...
  html += '<td style="padding:6px 16px; white-space:nowrap;">Payee</td><td style="padding:6px 16px; white-space:nowrap;">Description</td><td style="padding:6px 16px; white-space:nowrap;">Invoice #</td><td style="padding:6px 16px; white-space:nowrap;">Date</td><td style="padding:6px 16px; white-space:nowrap; text-align:right;">Amount</td><td style="padding:6px 16px; white-space:nowrap;">Check #</td><td style="padding:6px 16px; white-space:nowrap; text-align:right;">Action</td></tr></thead>';
  ...
  glGroup.invoices.forEach(inv => {
    ...
    html += '<td style="padding:6px 16px; white-space:nowrap;" title="' + ((inv.payee_name || inv.payee_code || '').replace(/"/g, '&quot;')) + '">' + (inv.payee_name || inv.payee_code || '—') + '</td>';
    html += '<td style="padding:6px 16px; max-width:260px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font-size:11px; color:var(--gray-600);" title="' + ((inv.notes || '').replace(/"/g, '&quot;')) + '">' + (inv.notes || '—') + '</td>';
    html += '<td style="padding:6px 16px; white-space:nowrap; font-family:monospace; font-size:11px;">' + (inv.invoice_num || '—') + '</td>';
    html += '<td style="padding:6px 16px; white-space:nowrap;">' + (inv.invoice_date ? inv.invoice_date.substring(0,10) : '—') + '</td>';
    html += '<td style="padding:6px 16px; white-space:nowrap; text-align:right; font-variant-numeric:tabular-nums;">$' + Math.round(inv.amount).toLocaleString() + '</td>';
    html += '<td style="padding:6px 16px; white-space:nowrap;">' + (inv.check_num || '—') + '</td>';
...
async function toggleInvoices(glCode, linkEl) {
    ...
    glGroup.invoices.forEach(inv => {
        ...
        html += '<td style="padding:6px 16px; white-space:nowrap; font-family:monospace; font-size:11px;">' + (inv.invoice_num || '—') + '</td>';
        html += '<td style="padding:6px 16px; white-space:nowrap;">' + (inv.invoice_date ? inv.invoice_date.substring(0,10) : '—') + '</td>';
        html += '<td style="padding:6px 16px; white-space:nowrap; text-align:right; font-variant-numeric:tabular-nums;">' + fmtAmt(inv.amount) + '</td>';
        html += '<td style="padding:6px 16px; white-space:nowrap;">' + (inv.check_num || '—') + '</td>';
        html += '<td style="text-align:right;">';
        if (isReclassed) {
            html += '<span style="font-size:11px; color:var(--orange);">→ ' + inv.reclass_to_gl + '</span> ';
