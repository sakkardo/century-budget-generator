...
<div class="wizard-container">
  ...
  <div class="wizard-content">
    ...
    <!-- Step 1: Building (Status UX Phase 4, 2026-06-09 — the duplicate
         Select Entity grid is RETIRED; the FA Dashboard is the one portfolio
         home and bare /wizard redirects there server-side. This step is just
         the building-context card for the per-building flow.) -->
    <div class="step-content active" data-step="1">
      <div class="step-header">
        <div class="step-badge">Step 1 of 5</div>
        <h1 class="step-title">Building</h1>
        <p class="step-description">Working on <span id="wizardBuildingName" style="font-weight:700; color:var(--gray-900);">&hellip;</span></p>
      </div>
      <div style="background:white; border:1px solid var(--gray-200); border-radius:10px; padding:18px 22px; max-width:560px;">
        <div style="font-size:13px; color:var(--gray-700); margin-bottom:12px;">The portfolio list lives on the FA Dashboard &mdash; one home for every building, with live file tiles, stages, and next steps. Pick a different building there.</div>
        <a href="/dashboard" style="display:inline-block; background:var(--blue); color:white; text-decoration:none; padding:8px 16px; border-radius:7px; font-weight:600; font-size:13px;">Open the dashboard &rarr;</a>
        <button type="button" onclick="showStep(2)" style="margin-left:8px; background:white; border:1px solid var(--gray-300); color:var(--gray-700); padding:8px 16px; border-radius:7px; font-weight:600; font-size:13px; cursor:pointer;">Continue to sources &rarr;</button>
      </div>
    </div>
...
// (Status UX Phase 4, 2026-06-09: the Select Entity grid and its ~800 lines
// of renderers/sort/filters/chips/hero/SP-scan plumbing were retired here.
// The FA Dashboard is the one portfolio surface; this template is now the
// per-building flow only. See STATUS_UX_PLAN.md Phase 4.)

// Select entity
function selectEntity(code, name) {
  selectedEntity = code;
  document.getElementById('railEntityName').textContent = name;
  const _bn = document.getElementById('wizardBuildingName');
  if (_bn) _bn.textContent = name + ' (' + code + ')';
  completeStep(1);
...
// Initialize (Status UX Phase 4: per-building flow only — the portfolio grid
// is retired; bare /wizard redirects to the dashboard server-side, so this
// page always loads with a building. Default landing is Step 2 (sources).)
document.addEventListener('DOMContentLoaded', () => {
  initializeData();
  updateRail();
  renderActionButtons();
  // The template var guarantees a building; run the entity-pick flow so the
  // chrome + async loads (sources, foundation, selections) kick off.
  if (selectedEntity) {
    const ent = (budgets || []).find(b => b.entity_code === selectedEntity);
    if (ent) selectEntity(selectedEntity, ent.building_name);
  }
  // ?step=N deep-link (default Step 2 — the building is already chosen).
  // Wrapped in setTimeout so it runs after selectEntity's async loads settle.
  try {
    const params = new URLSearchParams(window.location.search);
    const stepParam = parseInt(params.get('step') || '', 10);
    if (selectedEntity) {
      const stepN = (stepParam >= 1 && stepParam <= 5) ? stepParam : 2;
      setTimeout(() => {
        if (typeof showStep === 'function') showStep(stepN);
      }, 250);
    }
