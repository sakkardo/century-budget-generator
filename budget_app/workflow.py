...
async function faToggleInvoices(glCode, el) {
  ...
  html += '</div>';
  html += '<table style="width:100%; font-size:12px; border-collapse:collapse; background:white; border-radius:6px; box-shadow:0 1px 2px rgba(0,0,0,0.05);">';
  ...
  html += '<td style="padding:8px 12px; border-bottom:2px solid var(--gray-300); border-right:1px solid var(--gray-300); max-width:160px;">Payee</td><td style="padding:8px 12px; border-bottom:2px solid var(--gray-300); border-right:1px solid var(--gray-300); max-width:180px;">Description</td><td style="padding:8px 12px; border-bottom:2px solid var(--gray-300); border-right:1px solid var(--gray-300);">Inv #</td><td style="padding:8px 12px; border-bottom:2px solid var(--gray-300); border-right:1px solid var(--gray-300);">Date</td><td style="padding:8px 12px; text-align:right; border-bottom:2px solid var(--gray-300); border-right:3px solid #94a3b8;">Amount</td><td style="padding:8px 12px; border-bottom:2px solid var(--gray-300); border-right:1px solid var(--gray-300);">Check #</td><td style="padding:8px 12px; text-align:right; border-bottom:2px solid var(--gray-300);">Action</td></tr></thead>';
  ...
  glGroup.invoices.forEach(inv => {
    ...
    html += '<td style="padding:8px 12px; font-size:12px; max-width:160px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; border-bottom:1px solid var(--gray-200); border-right:1px solid var(--gray-200);" title="' + ((inv.payee_name || inv.payee_code || '').replace(/"/g, '&quot;')) + '">' + (inv.payee_name || inv.payee_code || '—') + '</td>';
    html += '<td style="padding:8px 12px; max-width:180px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font-size:12px; color:var(--gray-600); border-bottom:1px solid var(--gray-200); border-right:1px solid var(--gray-200);" title="' + ((inv.notes || '').replace(/"/g, '&quot;')) + '">' + (inv.notes || '—') + '</td>';
    ...
    html += '<td style="padding:8px 12px; white-space:nowrap; text-align:right; font-size:12px; font-weight:600; font-variant-numeric:tabular-nums; border-bottom:1px solid var(--gray-200); border-right:3px solid #94a3b8;">$' + Math.round(inv.amount).toLocaleString() + '</td>';
    html += '<td style="padding:8px 12px; white-space:nowrap; font-size:12px; border-bottom:1px solid var(--gray-200); border-right:1px solid var(--gray-200);">' + (inv.check_num || '—') + '</td>';
...
function renderEditableSheet(sheetName, sheetLines, contentDiv) {
  ...
  if (!document.getElementById('faSheetStyle')) {
    ...
    style.textContent = `
      ...
      .fa-invoice-detail .drill-sticky, .fa-grid .drill-sticky { position:sticky; left:321px; z-index:10; width:fit-content; min-width:850px; }
...
@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap');
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .invoice-detail-row .drill-sticky, .drill-sticky { position:sticky; left:321px; z-index:10; width:fit-content; min-width:850px; }
...
async function toggleInvoices(glCode, linkEl) {
    ...
    glGroup.invoices.forEach(inv => {
        ...
        html += '<td style="padding:8px 12px; white-space:nowrap; font-size:12px; border-bottom:1px solid var(--gray-200); border-right:1px solid var(--gray-200);">' + (inv.invoice_num || '—') + '</td>';
        html += '<td style="padding:8px 12px; white-space:nowrap; font-size:12px; border-bottom:1px solid var(--gray-200); border-right:1px solid var(--gray-300);">' + (inv.invoice_date ? inv.invoice_date.substring(0,10) : '—') + '</td>';
        html += '<td style="padding:8px 12px; white-space:nowrap; text-align:right; font-size:12px; font-weight:600; font-variant-numeric:tabular-nums; border-bottom:1px solid var(--gray-200); border-right:3px solid #94a3b8;">' + fmtAmt(inv.amount) + '</td>';
        html += '<td style="padding:8px 12px; white-space:nowrap; font-size:12px; border-bottom:1px solid var(--gray-200); border-right:1px solid var(--gray-200);">' + (inv.check_num || '—') + '</td>';
        html += '<td style="text-align:right;">';
        if (isReclassed) {
            html += '<span style="font-size:11px; color:var(--orange);">→ ' + inv.reclass_to_gl + '</span> ';
